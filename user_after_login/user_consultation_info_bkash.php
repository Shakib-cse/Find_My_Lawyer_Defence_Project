...
$chang_time_slot = $past_time_message = $er_0 = $chang_time_slot = $booked = $past_time_message = $time_1 = $time_3 = $time_3 =  $selected_slots_2 = $er_0 = '';

session_start();


$dataFromUrl = isset($_GET['additionalData']) ? $_GET['additionalData'] : '';
$_SESSION['additionalData'] = $dataFromUrl;
...
if (isset($_GET['data'])) {
    ...
    if ($lawyer_result->num_rows > 0) {
        $lawyer_data = $lawyer_result->fetch_assoc();
            ...
            $lawyer_fee_data = $lawyer_data["lawyer_fee"];

            $_SESSION['lawyer_fee_data'] = $lawyer_fee_data;
        }
} else {
...
if (isset($_POST['user_consulte'])) {
    ...
    if (isset($selected_slots[1])) {
        $selected_slots_2 = $selected_slots[1];
    }
    ...
    if ($selected_datetime->format('U') < $current_datetime->format('U')) {
        ...
        try {
            $checkAvailabilityQuery = "SELECT * FROM user_consulting 
                           WHERE (selected_slot LIKE '%$selected_slots_str%' OR selected_slot LIKE '%$selected_slots_1%' OR selected_slot LIKE '%$selected_slots_2%')
            ...
            if ($result->num_rows == 0) {
                ...
                if ($conn->query($user_consulting_sql) == TRUE) {
                    $conn->commit();
                    move_uploaded_file($pp_tmp_name, $pp_uploc);
                    $_SESSION['selected_slots_count'] = $selected_slots_count;
...
<head>
    ...
    <!-- css stylesheet link -->
    <link rel="stylesheet" href="../style/user_consulataion_info_bkash.css">
</head>
    ...
    <main style="padding-top:150px;padding-bottom:100px;background:wheat;">
        <div class="problem-info-area">
            <div class="container">
                ...
                <form action="user_consultation_info_bkash.php" method="POST" enctype="multipart/form-data">
                        <div class="item mb-3">
                            ...
                            if($lawyer_data["onlineOrOffline"] == 'both'){
                               ...
                               <div>
                                ...
                                <div class="radio-area">
                                <div>
                                    <label class="cursor-pointer" for="online">Online <input class="cursor-pointer" type="radio" name="online_offline" id="online" value="online" selected checked hidden required></label>
                                </div>
                            </div>
                            </div>';
                                ...
                                <div class="radio-area">
                                <div>
                                    <label class="cursor-pointer" for="offline">Offline <input class="cursor-pointer" type="radio" name="online_offline" id="offline" value="offline" selected checked hidden required></label>
                                </div>
                            </div>
                            </div>';
                             }
                        ...
                        <div class="item mb-3">
                            ...
                            <input type="date" id="start_date" name="start_date_user" required min="<?php echo date('Y-m-d'); ?>">

                        </div>
                        <div class="item mb-3">
                            <label class="d-block fs-5 text-decoration-underline mb-1 fw-bold" for="selected_slots">Select time slots for consultation:</label><br>
                        ...
                        <div class="text-black fs-2 fw-bold bg-info p-2">Payable Amount: <?php
                            echo $lawyer_fee_data;
                        ?> tk. (For one time slot)</div>
                        <input type="hidden" id="hidden_lawyer_email" name="hidden_lawyer_email">
                        <button type="submit" class="btn bg-success text-light mt-3 fw-bold" name="user_consulte">Payment with Bkash</button>
                    </form>
                </div>
            </div>
